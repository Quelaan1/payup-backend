...
class UserService:
    ...
    async def create_user(self, req_body: UserCreate):
        ...
        #     self.sessionmaker,
        #     lambda session: self._repo.create_obj(p_model=req_body, session=session),
        # )
        # with self.connect as conn:
        #     data = self._repo.create_user_item_2(p_model=req_body, conn=conn)
        #     logger.info("[dao response]: %s", data)
        #     return data
        ...
        # return run_transaction(
        #     self.connect,
        #     lambda session: self._repo.create_user_item_2(
        #         p_model=req_body, conn=session
        #     ),
        # )
    ...
    async def get_user(self, phone_number: str = None, user_id: UUID4 = None):
        """
