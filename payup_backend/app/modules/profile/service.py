...
from .model import ProfileDeleteResponse, ProfileUpdate, ProfileUpdateRequest
...
class ProfileService:
    ...
    async def update_user_profile(
        self, obj_id: UUID, update_model: ProfileUpdateRequest
    ):
        """
    ...
    async def delete_user_profile(self, obj_id: UUID):
        """
        Wraps a `session` call that deletes a profile.
        ...
        try:
            async with self.sessionmaker() as session:
                await self._repo.delete_obj(session=session, obj_id=obj_id)
                await session.commit()
