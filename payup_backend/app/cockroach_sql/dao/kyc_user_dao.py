...
class UserKycRelationRepo:
    ...
    def __init__(self):
        self._schema = UserKycRelationSchema
    ...
    async def get_by_related_obj(
        ...
        if not user_id is None:
            stmt = stmt.filter(self._schema.user_id == user_id)
        if not kyc_id is None:
            stmt = stmt.filter(self._schema.kyc_id == kyc_id)
        result = await session.execute(stmt)
    ...
    async def create_obj(
        ...
        try:
            db_model = self._schema(user_id=user_id, kyc_id=kyc_id)
    ...
    async def get_or_create_obj(
        ...
        stmt = (
            select(self._schema)
            .where(self._schema.kyc_id == kyc_id)
            .where(self._schema.user_id == user_id)
        )
        result = await session.execute(stmt)
        db_model = result.scalars().first()
    ...
    async def delete_obj(
        ...
        if user_id is not None and kyc_id is not None:
            if cond == "OR":
                stmt = stmt.where(
                    or_(self._schema.user_id == user_id, self._schema.kyc_id == kyc_id)
                )
            elif cond == "AND":
                stmt = stmt.where(
                    and_(self._schema.user_id == user_id, self._schema.kyc_id == kyc_id)
                )
            else:
                # Default behavior if cond is not specified but both IDs are provided
                stmt = stmt.where(self._schema.user_id == user_id).where(
                    self._schema.kyc_id == kyc_id
                )
        elif user_id is not None:
            stmt = stmt.where(self._schema.user_id == user_id)
        elif kyc_id is not None:
            stmt = stmt.where(self._schema.kyc_id == kyc_id)
        result = await session.execute(stmt)
        await session.flush()
