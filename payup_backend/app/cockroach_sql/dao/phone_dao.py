...
#     async def get_obj_by_user_id(self, session: AsyncSession, user_id: UUID):
#         """get phone by primary key"""
...
#             **p_model.model_dump(exclude=["m_pin", "m_number"], by_alias=True)  # type: ignore
#         )
#         db_model.m_number = str(p_model.m_number)  # type: ignore
...
#         if db_model is None:
#             raise NotFoundError(
#                 name=__name__, detail=BaseResponse(detail="Phone not found")  # type: ignore
#             )

#         update_data = p_model.model_dump(exclude=["m_pin"], exclude_unset=True)  # type: ignore
#         for key, value in update_data.items():
#             setattr(db_model, key, value)

#         if p_model.m_pin is not None:
#             db_model.pin = self.repo_schema.get_hash_password(
