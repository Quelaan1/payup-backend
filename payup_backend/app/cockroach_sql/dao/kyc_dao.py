...
class KycEntityRepo:
    ...
    async def get_kyc_by_entity_id(
        self, session: AsyncSession, entity_id: str, entity_type: KycType
    ) -> Optional[KycModel]:
        """Get KYC details by entity_id and entity_type"""
        try:
            # Encrypt the entity_id
            if entity_type == KycType.PAN:
                key = constants.PAYUP.PAN_KEY
            elif entity_type == KycType.AADHAAR:
                key = constants.PAYUP.UIDAI_KEY

            encrypted_entity_id = encrypt_entity_id(entity_id, key)

            # Query the kyc_entities table
            result = await session.execute(
                select(self.repo_schema)
                .where(self.repo_schema.entity_id_encrypted == encrypted_entity_id)
                .where(self.repo_schema.entity_type == entity_type.value)
            )

            db_model = result.scalars().first()

            if db_model:
                return KycCreate.model_validate(db_model)
            else:
                return None
