    ...
    def encode_keys(cls, data: Any) -> Any:
        if isinstance(data, dict):
            if "PAN_KEY" in data.keys():
                val = data.get("PAN_KEY") + "="
                data["PAN_KEY"] = val
            if "UIDAI_KEY" in data.keys():
                val = data.get("UIDAI_KEY") + "="
                data["UIDAI_KEY"] = val
        return data
...
class AttestrSettings(BaseSettings):
    """creates a singleton constants instance"""
    ...
    model_config = SettingsConfigDict(
        env_file=".env", env_prefix="attestr_", extra="ignore"
    )

    def __str__(self):
        return settings_as_string(self.model_dump(), "ATTESTR")
...
class Settings(BaseSettings):
    ...
    ATTESTR: AttestrSettings = AttestrSettings()
